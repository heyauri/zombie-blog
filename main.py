...
class Weibo(object):
    ...
    def print_user_info(self):
        ...
        if self.user.get('verified_type'):
            print("认证类型: " + str(self.user['verified_type']))
        if self.user.get('verified_reason'):
            print("认证原因: " + str(self.user['verified_reason']))
    ...
    def parse_retweet_wb_simply(self, weibo_info):
        weibo = OrderedDict()
        if weibo_info['user']:
            weibo['user_id'] = weibo_info['user']['id']
            weibo['screen_name'] = weibo_info['user']['screen_name']
    ...
    def get_one_weibo(self, info):
        ...
        try:
            ...
            if retweeted_status:  # 转发
                ...
                if self.filter:
                    retweet = self.parse_retweet_wb_simply(retweeted_status)
                else:
                    if is_long:
